...
### Live 镜头 baseline(有脸,300/300 帧检出,2026-06-12 筆電)

| 阶段 | mean | p50 | p95 | max |
|---|---|---|---|---|
| 读帧(camera read) | **20.23ms** | 18.96ms | 50.17ms | 66.73ms |
| FaceLandmarker 推理 | 13.01ms | 13.06ms | 14.46ms | 16.25ms |
| EAR + 状态机 | 0.14ms | 0.14ms | 0.23ms | 0.53ms |
| **每帧总计** | 33.39ms | 32.14ms | 62.34ms | 79.93ms |

端到端 FPS:**29.9**(与 live 验收的 30~32 一致,量测工具可信)

**反直觉发现:筆電上瓶颈是「镜头」,不是「模型」。**

- 30fps 镜头每 ~33ms 才出一帧;推理只要 13ms,跑完就空等下一帧
  → `read_frame` 的 20ms 其实是「等镜头」的阻塞时间
- 纯计算成本仅 ~13.2ms/帧 → 若镜头不设限,这台筆電理论可跑 **~75 FPS**
- 有脸 vs 无脸推理:13.0ms vs 7.5ms → landmark 模型本身约 5.5ms

**对 Pi 部署的意义:**
- Pi 5 上推理会慢 3~5 倍(预估 40~65ms),届时瓶颈会从镜头转移到模型,
  量化才开始有意义 —— 这正是「先量测、再优化」的理由
- 量测方法学:`benchmark.py` 的分阶段拆解能直接看出瓶颈在哪一段,
  避免「在镜头瓶颈的机器上优化模型」这种无效功
